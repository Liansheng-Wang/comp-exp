...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    pcl::PointCloud<hesai_ros::Point>::Ptr pl_orig(new pcl::PointCloud<hesai_ros::Point>());
    pcl::fromROSMsg(*msg, *pl_orig);
    int plsize = pl_orig->points.size();
//    ROS_INFO("cloud input size: %d", plsize);
    ...
    for (int i = 0; i < plsize; i++)
    {
      ...
      // cout<<"!!!!!!"<<i<<" "<<plsize<<endl;
      added_pt.x = -orig_pt.y;
      added_pt.y = orig_pt.z;
      added_pt.z = orig_pt.z;
      added_pt.intensity = orig_pt.intensity;
      added_pt.curvature = (orig_pt.timestamp - time_begin) * 1000;  // curvature unit: ms // cout<<added_pt.curvature<<endl;
