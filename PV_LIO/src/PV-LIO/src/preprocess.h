...
enum LID_TYPE{AVIA = 1, VELO16, OUST64, HESAI, NCLT}; //{1, 2, 3}
...
// clang-format off
POINT_CLOUD_REGISTER_POINT_STRUCT( nclt_ros::Point,
                                  (float, x, x)
                                  (float, y, y)
                                  (float, z, z)
                                  (float, time, time)
...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  void nclt_handler(const sensor_msgs::PointCloud2::ConstPtr &msg);
