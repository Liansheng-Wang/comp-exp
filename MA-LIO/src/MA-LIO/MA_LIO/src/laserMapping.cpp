...
void h_share_model(state_ikfom &s, esekfom::dyn_share_datastruct<double> &ekfom_data)
{
    ...
    // 得到有效点之后，进行雅可比计算，在计算雅可比的过程中，把有效点部分的协方差矩阵给算出来.
    ...
    for (int i = 0; i < effct_feat_num; i++)
    {
        ...
        if (lid_idx != 0)
            point_this_be = extrinsic_quat[0].conjugate() * 
                                ((kf.temporal_comp[lid_idx - 1].q_ * 
                                    (extrinsic_quat[lid_idx] * 
                                        point_this_be + extrinsic_trans[lid_idx]) 
                                            + kf.temporal_comp[lid_idx - 1].t_) 
                                                - extrinsic_trans[0]);
        ...
        if (extrinsic_est_en)
        {
            ...
            evalPointUncertainty(laser_p, cov, pose_unc[lid_idx][uncertain]);  // laser_p 涵盖了 effect 的部分. 
            ekfom_data.R(i, 0) = cov(0, 0) + cov(1, 1) + cov(2, 2);
            laser_p.normal_y = cov(0, 0) + cov(1, 1) + cov(2, 2);      // 点的不确定性只保存了 cov 的 trace
    ...
    // 这个过程是给给个点分配一个 R, R 是 点的 noise. 但是这里其实是有点问题的，noise 对于点面 ICP 只应该分布在面的法向上。
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_lidar = nh.subscribe(lid_topic[0], 100, lidar_cbk_);
            ...
            for (int num = 0; num < lid_num; num++)
            {
                ...
                else
                {
                    for (int i = 0; i < (int)kf.lidar_uncertainty[num].size() - 1; i++)
                    {
                        ...
                        // pose_unc[num].push_back(kf.lidar_uncertainty[num][i]); // 这里我提的issue，改为下一个
